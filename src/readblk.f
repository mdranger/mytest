...
c...Local variables for new input block
      ...
      real readreal
      logical dbgp
      character buffr*90, readstring*20
...
!...Reset the dtpl value; the units are years, but it should
!     have a value representing an integral number of months as
!     accurately as possible.
      if (dtpl .lt. .01) then
!...Reset to monthly if lower than lowest limit
        dtpl = dt
      else
!...Rounding
        modt = int(12.*dtpl+.05)
...
c     use blknum to mark an error  (blknum < 0  bad input)
     ...
     $         ' rotation length ',timary(ttlind, 1), buffr
         ...
         blknum = -abs(blknum)
       elseif ((timary(ttlind,2).lt.0).or.(timary(ttlind,2).gt.12)) then
     ...
     $          (timary(ttlind-1,2).gt.timary(ttlind,2)  )))     then
           write(*,'(a/a)') ' fatal error: dates out of sequence ',buffr
...
c
!...Add FLOD for flooding event, to calculate the wetland
        if(INDEX('CROP CULT FERT FIRE GRAZ HARV IRRI OMAD TREE TREM 
     $     FLOD', cmdary(ttlind)(1:4)) .ne. 0) then
c
