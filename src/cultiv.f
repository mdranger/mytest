...
c...Some standing dead goes into surface litter
      if (stdedc .gt. 0.0) then
        ...
        do 10 iel = 1, nelem
          recres(iel) = stdede(iel)/(0.0001+stdedc)
10      continue
        fr14 = stdcis(LABELD)/(0.0001+stdedc)
...
c...Some above ground live goes into surface litter
      if (aglivc .gt. 0.0) then
        ...
        do 50 iel = 1, nelem
          recres(iel) = aglive(iel)/(0.0001+aglivc)
50      continue
        fr14 = aglcis(LABELD)/(0.0001+aglivc)
...
!      print *, "oUT", pltlig(BELOW), fr14, bglcis
      endif
