...
c...This is for use in partit.
      do 50 iel = 1, MAXIEL
        if (rcelit(SRFC, iel) .gt. 0.) then
          reclt1(iel) = 1. / (0.0001+rcelit(SRFC, iel))
        endif
50    continue
      do 55 iel = 1, MAXIEL
        if (rcelit(SOIL, iel) .gt. 0.) then
          reclt2(iel) = 1. / (0.0001+rcelit(SOIL, iel))
        endif
55    continue
      ...
      if (initre .ne. ' ') then
        do 80 iel = 1, MAXIEL
          if (cerfor(IVAL,FBRCH,iel) .gt. 0.) then
                    wood1e(iel)=wood1c/(0.0001+cerfor(IVAL,FBRCH,iel))
          endif
          if (cerfor(IVAL,LWOOD,iel) .gt. 0.) then
                    wood2e(iel)=wood2c/(0.0001+cerfor(IVAL,LWOOD,iel))
          endif
          if (cerfor(IVAL,CROOT,iel) .gt. 0.) then
                    wood3e(iel)=wood3c/(0.0001+cerfor(IVAL,CROOT,iel))
          endif
80      continue
      endif
